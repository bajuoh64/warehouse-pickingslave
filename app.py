...
with st.container():
    ...
    for i in range(st.session_state.picker_count):
        ...
        with seg_cols[i]:
            ...
            if st.button(f"picker_{idx}", key=f"seg_btn_{idx}", use_container_width=True):
                st.session_state.active_picker = idx
            st.markdown("""
            <style>button[kind="secondary"][data-testid^="baseButton-secondary"]{text-indent:-9999px;height:0;padding:0;margin:0;border:none;box-shadow:none;}</style>
            """, unsafe_allow_html=True)
...
st.markdown(
    ...
    f"<div class='meta-qty'>{data['qty']}</div></div>", unsafe_allow_html=True
)
...
# 3) First in Category | Last in Category (셋째줄 좌/우)
